...
void des_encrypt(string path)
{
    ...
    int extend_size = file_size;
    if (file_size % 8 != 0)
    {
        extend_size = (file_size / 8 + 1) * 8;
    }
    unsigned char* psrc = new unsigned char[extend_size];

    memset(psrc, 0, extend_size);
    fread(psrc, 1, file_size, fp);
    fclose(fp);

    uint64_t key = *(uint64_t*)"hellodes";
    uint64_t input;

    unsigned char* sig = psrc;
    while (psrc - sig < file_size)
    {
        input = *(uint64_t*)psrc;
        *(uint64_t*)psrc = des(input, key, 'e');
        psrc += 8;
    }
    string newpath = replace_distinct(path, ".xml", ".db");
    fp = fopen(newpath.c_str(), "wb");
    fwrite(sig, 1, extend_size, fp);
    fclose(fp);
}

int get_xml_real_size(unsigned char* buf,int file_size)
{
    int i= 0;
...
void des_decrypt(string path)
{
    ...
    unsigned char* psrc = new unsigned char[file_size];
    memset(psrc, 0, file_size);
    fread(psrc, 1, file_size, fp);
    fclose(fp);
    ...
    {
        input = *(uint64_t*)psrc;
        *(uint64_t*)psrc =des(input, key, 'd');
        psrc += 8;
    }

    string newpath = replace_distinct(path, ".db", ".xml");
    fp = fopen(newpath.c_str(), "wb");
    int real_size = get_xml_real_size(sig, file_size);
    fwrite(sig, 1, real_size, fp);
    fclose(fp);
}
